...
export function retrieveError(retrieveError) {
  return {
    type: 'USER_UPDATE_RETRIEVE_ERROR', retrieveError,
  };
}

export function retrieveLoading(retrieveLoading) {
  return {
    type: 'USER_UPDATE_RETRIEVE_LOADING', retrieveLoading,
  };
}

export function retrieveSuccess(retrieved) {
  return {
    type: 'USER_UPDATE_RETRIEVE_SUCCESS', retrieved,
  };
}
...
export function updateError(updateError) {
  return {
    type: 'USER_UPDATE_UPDATE_ERROR', updateError,
  };
}

export function updateLoading(updateLoading) {
  return {
    type: 'USER_UPDATE_UPDATE_LOADING', updateLoading,
  };
}

export function updateSuccess(updated) {
  return {
    type: 'USER_UPDATE_UPDATE_SUCCESS', updated,
  };
}
...
export function update(item, values) {
  return (dispatch) => {
    ...
    return fetch(item['@id'], {
      ...
      .then(response => response.json())
      .then((data) => {
        // toast.success(i18n.t('companiesUpdateSuccess'));
